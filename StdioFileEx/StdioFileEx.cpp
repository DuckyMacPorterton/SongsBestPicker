...
//
//  We don't want to write the BOM for UTF8

//#define WriteBOMForUTF8
...
CStdioFileEx::CStdioFileEx(): 
	...
	m_nFileCodePage(CP_UTF8),  // -1
	m_nFlags(0),
	m_nByteOrderMarkBytesInFile(0),
	m_cUnicodeFillerChar(sDEFAULT_UNICODE_FILLER_CHAR), 
	m_bWriteBOM(true), // By default, write the BOM
	CStdioFile()
{
	memset (&(m_arrMultibyteDefaultBuffer[0]), 0, sizeof(m_arrMultibyteDefaultBuffer[0]) * VPIE_MAX_NUM_CHARS_TO_READ);
	memset (&(m_arrUnicodeDefaultBuffer[0]), 0, sizeof(m_arrUnicodeDefaultBuffer[0]) * VPIE_MAX_NUM_CHARS_TO_READ);
}
	...
	CStdioFile(lpszFileName, nOpenFlags)
{
	...
	//
	//  Doug added this 2/19/2010 as it wasn't handling writing unicode
	//  properly when opening the file in this constructor
...
#ifndef _UNICODE
	//
	//  This is to try to handle UTF8 (and whatnot) files in a non-unicode
	//  environment.  It won't always get it right, but hopefully it will
	//  avoid InvalidArgument errors.

	_setmbcp (nCodePage);

	CString str; str.Format (_T(".%u"), nCodePage);
...
{
	m_bWriteBOM = bWrite;
	m_bForceBOMSetting = true;	// this means to never change m_bWriteBOM on our own because we've set it where we want it
}
...
BOOL CStdioFileEx::Open(LPCTSTR lpszFileName,UINT nOpenFlags,CFileException* pError /*=NULL*/)
{
	...
	{
		//
		//  If we're writing a file, we want different default behavior for UTF-8 than UTF-16.  
		//  UTF-16 defaults to always writing BOM.
		//  UTF-8 defaults to not writing BOM
		//
		//  Caller can still call SetWriteBOM before Opening file.

		if ((nOpenFlags & CFile::modeWrite) || (nOpenFlags & modeReadWrite))
		{
			//
			//  If the user has called SetWriteBOM then we make a note in m_bForceBOMSetting, so
			//  we don't change that setting here

			if (!m_bForceBOMSetting)
			{
...
//		if (!(nOpenFlags & CFile::modeCreate) && (nOpenFlags & CFile::modeRead || nOpenFlags & CFile::modeReadWrite))
		if (!(nOpenFlags & CFile::modeCreate) && !(nOpenFlags & CFile::modeWrite ))
		{
			...
			if (m_bIsUnicodeText)
				m_nByteOrderMarkBytesInFile = sizeof (cBOM);
			...
			{
				//
				//  Since it doesn't have a unicode byte order mark, see if we can
				//  figure out what 



#define AutoDetectCodePage
#ifdef AutoDetectCodePage
				UINT nCodePage = 0;
				if (DetectCodePage(nCodePage))
				{
					SetCodePage(nCodePage);
				}
#endif
				//
				//  See if it has a UTF-8 byte order mark
				...
				if (CP_UTF8 == nCodePage)
				{
					...
					bool bBOMHere = false;
					for (int i = 0; i < sizeof(UTF8_BOM); i++)
					{
						if (arrUTF8BOMTest[i] == UTF8_BOM[i])
							bBOMHere = true;
						else
							break;
					} // end loop checking for utf8 bom

					if (bBOMHere)
						m_nByteOrderMarkBytesInFile = sizeof(UTF8_BOM);
					SeekToBegin();

				} // end if is utf-8
			} // end if is not UTF-16

			m_bCheckFilePos = true;
		} // end if opening read file
	}
...
/////////////////////////////////////////////////////////////////////////
//
//  If users passes in a text qualifier (like " ), then if it finds a
//  newline inside a text qualifier, it skips it and keeps reading.
//
BOOL CStdioFileEx::ReadString (CString& rString, UINT nMax /* = 0 */, CString strTextQualifier /* = L"" */)
{
	ASSERT(m_pStream != NULL);

	BOOL			bReadData = FALSE;

	// If at position 0, discard byte-order mark before reading. To optimize reading, we only
	...
	if (m_bCheckFilePos && GetPosition() == 0)
	{
		...
		{
			//
			//  If the UTF8 byte order mark is here, get rid of it.
			//  Otherwise, well, we don't want to skip the first 3 characters
	...
	if (m_bIsUnicodeText)
	{
		...
		if (strTextQualifier.IsEmpty ())
			bReadData = ReadUnicodeLine (rString, nMax);
		else
		{
			CString strFullLine;
			do 
			{
				...
				//
				//  Make sure our newline isn't inside a text qualifier.  
				//  Also make sure we're not checking if our text qualifier is inside one.  For example, 
				//  if text qualifier is "  then if we read our line, which ends in just \n, then the last char
				//  might be a " if it's the end of a column in csv or something.  So if we check if " is inside ""'s it will
				//  be, since it is, itself, the end of the text qualifiying.  
	...
	{
		if (strTextQualifier.IsEmpty ())
			bReadData = ReadMultiByteLine(rString, nMax);
		else
		{
			CString strFullLine;
			do 
	...
	return bReadData || !rString.IsEmpty ();	// In case we had multiple reads and the first got data but a later one didn't
}
...
/*virtual*/ LPTSTR CStdioFileEx::ReadString(LPTSTR lpsz,UINT nMax)
{
	...
	//
	//  Doug is re-enabling it because when we're reading a # chars we don't really
	//  care what that number it, we just do it to prevent it from reading a multi megabyte 
	//  file that has no newlines all in one string.  So if it's unicode and we don't
	//  get the # chars we get with MBCS it doesn't matter.  -- dlp 3/5/08
...
void CStdioFileEx::WriteString(LPCTSTR lpsz)
{
	...
	if (m_bWriteBOM && (!m_pStream || GetPosition() == 0))
	{
		...
		//
		//  Note that we default to NOT writing a BOM for UTF-8.  To do so, caller calls SetWriteBOM before
		//  writing first string (or before opening)
...
UINT CStdioFileEx::ProcessFlags(UINT& nOpenFlags)
{
	...
	//
	//  I'm not positive this is a good idea, but it seems a lot better than
	//  forcing us to create a conditional #ifdef _UNICODE every time we use
	//  this class.  The question is, do we ever want to write a non-unicode file
	//  if we're a unicode app?  Maybe UTF8?

//#ifdef _UNICODE
//	if (nOpenFlags & CFile::modeWrite)
//		nOpenFlags |= modeWriteUnicode;
//#endif
...
//
//
ULONGLONG CStdioFileEx::GetLength ()
{
	//
	// We don't trust CStdioFile::GetLength on files > 2gb
...
/*virtual*/ STDIOEXLONG CStdioFileEx::Seek(LONGLONG lOff,UINT nFrom)
{
	...
	if ((lOff < LLONG_MIN) || (lOff > LLONG_MAX))
	{
...
ULONGLONG CStdioFileEx::GetPosition ()
{
	//
	//  This is stolen from CStdioFile::GetPosition and
	//  changed to use _ftelli64

	ASSERT_VALID(this);
	ASSERT(m_pStream != NULL);

	__int64 pos = _ftelli64 (m_pStream);
	if (pos == -1)
...
/*static*/ int CStdioFileEx::GetRequiredMultiByteLengthForUnicodeString(IN const wchar_t * szUnicodeString,IN UINT nCodePage /*=CP_ACP OPTIONAL*/)
{
	...
	try 
	{
		...
		nBytesNeeded = WideCharToMultiByte((UINT)nCodePage, 
			(nCodePage==CP_UTF8 ? 0 : WC_COMPOSITECHECK | WC_SEPCHARS), // Flags
			szUnicodeString,
			-1, // Process entire input string, including terminator.
			NULL, 
...
BOOL CStdioFileEx::ReadUnicodeLine(OUT CString& sOutputLine, UINT nMax /* = 0 */)
{
	BOOL			bReadData = FALSE;
	wchar_t*		pszUnicodeString = (wchar_t*)&m_arrUnicodeDefaultBuffer;
	bool			bNeedToDelete = false;

	if (nMax > nSTDIOFILEEX_DEFAULT_BUFFER_SIZE)
	{
		pszUnicodeString = new wchar_t[nMax + 1];
		bNeedToDelete = true;
	}

	try
	{
		bReadData = (FGGETS_OK == fggets<wchar_t>(&pszUnicodeString, m_pStream, bNeedToDelete, nMax, nMax == 0)); //nSTDIOFILEEX_DEFAULT_BUFFER_SIZE));
...
BOOL CStdioFileEx::ReadMultiByteLine(OUT CString& sOutputLine, UINT nMax /* = 0 */)
{
	BOOL			bReadData = FALSE;
	...
	if (nMax > nSTDIOFILEEX_DEFAULT_BUFFER_SIZE)
	{
		pszMultiByteString = new char[nMax + 1];
		bNeedToDelete = true;
	}
	...
	try
	{
		...
		// to the line length
		bReadData = (FGGETS_OK == fggets<char>(&pszMultiByteString, m_pStream, bNeedToDelete, nMax, nMax == 0)); // nSTDIOFILEEX_DEFAULT_BUFFER_SIZE));

		if (bReadData)
		{
			//
			//  Make sure that the last character we read is a complete character.  That is, if it's a UTF-8 3-byte character,
			//  we don't want to just read the first 2 bytes. 

			ULONGLONG nLen = strlen (pszMultiByteString);
			if ((0 == nMax) || (nLen == (nMax - 1)))
			{
				//
				...
				{
					//
					//  This means some of our bytes are not valid.  So remove those from our buffer and
					//  roll back our file pointer
			...
			} // end if we filled our whole buffer
...
void CStdioFileEx::WriteMultiByteLine(IN LPCTSTR sInputLine)
{
	...
	else
	{
		...
		CFile::Write((const void*)sInputLine, (int) strlen ((const char*)sInputLine)*sizeof(char));
	}
}
...
//////////////////////////////////////////////////////////////////////
//
//    D E T E C T   C O D E   P A G E 
//
//  Tries to guess the code page.  Returns true if we have a guess,
//  false if not.
//
bool CStdioFileEx::DetectCodePage (UINT& rnCodePage)
{
	//
	//  We want to leave the file in the same place as we found it

	LONGLONG nPos = GetPosition ();

	//
	//  The more data we have, the more accurate the guess will be

#define CODE_PAGE_READ_SIZE		65536
	char arrBuffer[CODE_PAGE_READ_SIZE];
	UINT nBytesRead = Read (&(arrBuffer[0]), CODE_PAGE_READ_SIZE);
	if (nBytesRead < 1)
		return false;
