...
//#define	nSTDIOFILEEX_DEFAULT_BUFFER_SIZE	65536 // 32768 // 4096			// Size of default buffer for strings

#define nSTDIOFILEEX_DEFAULT_BUFFER_SIZE	VPIE_MAX_NUM_CHARS_TO_READ
...
public:
	...
	virtual BOOL			ReadString (CString& rString, UINT nMax = 0, CString strTextQualifier = L"");
//	virtual LPTSTR			ReadString(LPTSTR lpsz, UINT nMax = 0);
	...
	UINT				GetCodePage () {return m_nFileCodePage;};
...
//																			 IN UINT nCodePage=CP_ACP OPTIONAL);
	...
	int		GetByteOrderBytesInFile () {return m_nByteOrderMarkBytesInFile;};

protected:
	UINT	ProcessFlags(UINT& nOpenFlags);//, bool& bCheckForUnicodeOnOpen);
#ifdef _UNICODE
	BOOL	ReadUnicodeLine(OUT CString& sOutputLine, UINT nMax = 0);
	BOOL	ReadMultiByteLine(OUT CString& sOutputLine, UINT nMax = 0);
	...
	bool		m_bWriteBOM;
	bool		m_bForceBOMSetting = false;
//	bool		m_bReadBOM
	int			m_nByteOrderMarkBytesInFile;

	wchar_t	m_arrUnicodeDefaultBuffer[VPIE_MAX_NUM_CHARS_TO_READ];	// default buffer for strings; used to avoid dynamic allocation where possible
	char		m_arrMultibyteDefaultBuffer[VPIE_MAX_NUM_CHARS_TO_READ]; // default buffer for strings; used to avoid dynamic allocation where possible
