...
BOOL CMonitor::IsOnMonitor( const LPRECT lprc, int nRequireThisManyPixelsToBeOnMonitor /* = 15 */) const
{
	CRect rectMonitor;
	GetMonitorRect (rectMonitor);

	CRect rcIntersect;
	rcIntersect.IntersectRect (lprc, rectMonitor);

	if ((rcIntersect.Width () >= nRequireThisManyPixelsToBeOnMonitor) &&
		(rcIntersect.Height () >= nRequireThisManyPixelsToBeOnMonitor))
		return true;
...
//
//  Doug added this one
BOOL CMonitor::IsCompletelyOnMonitor (const LPRECT lprc) const
{
	CRect rcMonitor;
	...
	rcIntersect.IntersectRect (rcMonitor, lprc);

	//
	//  If the intersection is the same as the passed in rectangle,
	//  it means it's all on the monitor
...
////////////////////////////////////////////////////////////////////////////////
//
//  Returns the shortest distance from rcWnd to  this monitor for each X and Y.
//
//
BOOL CMonitor::GetDistanceFromMonitor (CRect rcWnd, int& rdX, int& rdY)
{
	CRect rcMonitor;
	GetMonitorRect (rcMonitor);

	int nMinX = abs (rcWnd.left - rcMonitor.left);
	nMinX = min (nMinX, abs (rcWnd.left - rcMonitor.right));
	nMinX = min (nMinX, abs (rcWnd.right - rcMonitor.right));
	nMinX = min (nMinX, abs (rcWnd.right - rcMonitor.left));


	int nMinY = abs (rcWnd.top - rcMonitor.top);
	nMinY = min (nMinY, abs (rcWnd.top - rcMonitor.bottom));
	nMinY = min (nMinY, abs (rcWnd.bottom - rcMonitor.bottom));
	nMinY = min (nMinY, abs (rcWnd.bottom - rcMonitor.top));
...
void CMonitor::ClipRectToMonitor( LPRECT lprc, const BOOL UseWorkAreaRect ) const
{
    ...
    lprc->right = lprc->left + min (w, rect.Width ()); // doug changed, was + w
    lprc->bottom = lprc->top  + min (h, rect.Height ()); // doug changed, was + h
}
