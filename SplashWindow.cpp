...
const CString CSplashWindow::m_productNameFontName = L"Arial";	// name of font for application name
...
const CString CSplashWindow::m_bodyFontName = L"Arial";			// name of font for company name, copyright and version	
...
const CString CSplashWindow::m_statusMessageFontName = L"Arial";	// name of font for status message while starting	
...
{	
	...
	wchar_t *buf;				// pointer to buffer to receive file-version info.
	wchar_t buf1[1024];				// pointer to buffer to store version string.
	CString nameStr;
	nameStr.Format(L"%s.EXE", AfxGetApp()->m_pszExeName);
	wchar_t *name = (wchar_t *)((const wchar_t *)nameStr);
	...
	if (verSize != 0)
	{
		...
		if ( doThis ) {
			wcsncpy(buf1, (wchar_t *)lplpBuffer, ver);
		...
		if ( doThis ) {
			...
			int numberOfValues = swscanf ( buf1, L"%d.%d.%d.%d", &versionNumbers[0],&versionNumbers[1], &versionNumbers[2], &versionNumbers[3] );
			CString number;
			m_versionNumberString = "";

#ifdef ThatWay
			for ( int i=0; i<numberOfValues; i++ ) {
				if (bFoundNonZero && m_versionNumberString.IsEmpty() && (versionNumbers[i] == 0) )
					continue;
				if ( !m_versionNumberString.IsEmpty() )
					m_versionNumberString += ".";
				number.Format(L"%d", versionNumbers[i] );
				m_versionNumberString += number; 
			}
#else
			m_versionNumberString.Format (L"%d.%d", versionNumbers[0], versionNumbers[1]);
#endif
			m_versionString = L"Version " + m_versionNumberString;
